...
var fixedtoc = (function ($) {
	 ...
	 * @return float
	 */
	function getFixedHeight(ele) {
		if (ele.length && 'fixed' == ele.css('position')) {
			return parseToInt(ele.outerHeight());
		}
	...
	function createElements() {
		$e = {
			...
			list: $('#ftwp-list'),
			postStart: $('#ftwp-postcontent-start'),
			postEnd: $('#ftwp-postcontent-end')
		};
	...
	var dataObj = (function () {
		...
		var setPostRect = function () {
			...
			var startOffset = $e.postStart.offset();
			var endOffset = $e.postEnd.offset();

			data.postRect.left = startOffset.left;
			data.postRect.top = startOffset.top;
			data.postRect.width = $e.postStart.outerWidth();
			data.postRect.right = data.postRect.left + data.postRect.width;
			data.postRect.bottom = endOffset.top;
		...
		var hint = function () {
			...
			$.each(headingsTop, function (i) {
				...
				if (this.headingTop <= scrollTop && headingsTop[i + 1].headingTop > scrollTop) {
					prevHeading = [this, headingsTop[i + 1]];
	...
	var controlObj = (function () {
		...
		var onLoad = function () {
			...
			if (conditionObj.fixedWidget()) {
				...
				//				actionObj.register( 'fixedWidget', noScrollWindow );
			}
			...
			//			actionObj.register( 'fixedToPost', noScrollWindow );
